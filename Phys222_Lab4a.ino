...
void logTemps() {
  ...
  if (currCelcius > tempHot) {
    if (((currTime - prevTime) / 1000) % 10 == 0) {
      Serial.println("Booyaw!");
      PrevTime = currTime;
    }
    if ((prevTime / 1000) % 10 == 0 && (currCelcius - celcius10SecAgo >= 2.0)) {
      digitalWrite(ledHotPin, HIGH);
      celcius10SecAgo = currCelcius;
    }
    digitalWrite(ledWarmPin, LOW); 
  }
  ...
  Serial.println(prevTime / 1000.0);  
  ...
  Serial.print("\tCelcius 10 seconds ago: \t");
  Serial.println(celcius10SecAgo);  
